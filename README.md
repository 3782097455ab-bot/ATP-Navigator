...
访问 `http://127.0.0.1:8501`。界面统一提供候选浏览、证据矩阵、Glide/Vina协议比较、冻结决策输出、预算感知证据获取、生成结构谱系、任务与工具能力、研究对话、实验反馈空接口和带审计元数据的导出。它不重训模型、不生成实验结果，也不自动运行不可用后端。详见 [Phase18A用户指南](docs/User_Guide.md)。
