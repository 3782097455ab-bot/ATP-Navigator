...
## 2026-09-02：研序智航中文小范围公测界面

- 目标：把既有研究工作区从开发者导向页面升级为面向科研使用者的中文公测产品界面，不训练模型、不改科学结果；
- 产品命名：面向用户名称确定为“研序智航”；原`ATP-Navigator`仅作为历史仓库、数据与模型资产代号继续保留；
- 界面变化：采用对话主入口与科研工作区并列结构，新增首次使用教程、全中文导航、统一中文表头、新建研究会话入口和深墨/生命绿/科技蓝视觉系统；
- 功能保留：候选库、证据矩阵、三协议比较、冻结/影子决策、证据获取、3D结构、计算任务、实验反馈、团队复核、活动记录、迭代闭环、基准资源和导出均继续读取既有Registry；
- 本地部署：新增`scripts/start_yanxu_zhihang.ps1`，当前入口`http://127.0.0.1:8501`，不设置日期到期；
- 验收：智能研究助手中文查询成功返回真实协议分歧记录；7P3W登记Vina构象可交互显示；212项完整回归测试通过；
- 模型与数据变化：无。未训练、覆盖或晋升Model v0–v4-alpha；未修改Decision Engine权重、历史计算数值或实验状态。
