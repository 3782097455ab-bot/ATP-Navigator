# 研序智航（历史仓库：ATP-Navigator）当前系统状态

更新时间：2026-09-02

## 中文小范围公测界面

- 面向用户的产品名称更新为“研序智航”；`ATP-Navigator`只保留为历史仓库、数据集和模型版本代号；
- 本地入口为`http://127.0.0.1:8501`，使用`scripts/start_yanxu_zhihang.ps1`可幂等启动，进程不设置日期到期时间；
- 已完成全中文对话式主入口、分组科研功能导航、首次使用教程、统一中文表头、竞赛标识同系深墨/生命绿/科技蓝视觉；
- 17个既有功能页面全部保留，未修改Model v0–v4-alpha、Decision Engine、Registry科学记录或冻结协议；
- 智能研究助手真实查询“Glide和Vina分歧最大的10个候选”成功；候选、协议、决策和7P3W三维页面完成本地浏览器验收；
- 212项完整回归测试通过。当前仍属于实验前计算证据整合与候选优先级辅助决策系统，不扩展临床或生物安全叙事。
